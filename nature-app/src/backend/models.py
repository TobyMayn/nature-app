...
class Results(SQLModel, table=True):
    ...
    analysis_type: str
    request_params: JSON # This is includes the parameters from AnalysisBody. SQLModel does not support adding it as a type. So we set type to JSON.
    status: str = Field(default="Pending")
    requested_at: datetime
    completed_at: datetime | None = Field(default=None)
    error_message: str | None = Field(default=None)
    result: JSON | None = Field(default=None)
